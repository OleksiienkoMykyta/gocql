...
func Marshal(info TypeInfo, value interface{}) ([]byte, error) {
	if info.Version() < internal.ProtoVersion1 {
...
func writeCollectionSize(info CollectionType, n int, buf *bytes.Buffer) error {
	if info.proto > internal.ProtoVersion2 {
		if n > math.MaxInt32 {
...
func marshalList(info TypeInfo, value interface{}) ([]byte, error) {
	...
	case reflect.Slice, reflect.Array:
		...
		for i := 0; i < n; i++ {
			...
			if item == nil && listInfo.proto > internal.ProtoVersion2 {
				itemLen = -1
			}
...
func marshalMap(info TypeInfo, value interface{}) ([]byte, error) {
	...
	for _, key := range keys {
		...
		if item == nil && mapInfo.proto > internal.ProtoVersion2 {
			itemLen = -1
		}
